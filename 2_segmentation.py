...
fn = 0
TARGET_FILE_PATH = 'result/characters/'
INPUT_IMAGE = cv.imread('sample_images/d1.png')
...
def findCapPoints(img):
    ...
    for i in range(img.shape[1]):
        ...
        while k > 0:
            if col[k - 1] == 255:
                dpoints.append((i, k - (0.1 * k)))
                break
            k -= 1

        for j in range(col.shape[0]):
            if col[j] == 255:
                cpoints.append((i, j - (0.3 * j)))
                break
...
def baselines(letter2, upoints, dpoints):
    ...
    # show upper and lower basepoints ...
...
# MAIN ...
try:
    textLines = lineSegment(INPUT_IMAGE)
    ...
    for letterGray in imgList:
        ...
        min_round_letter_threshold = 180
        ...
        for i in wordImgList:
            if i.shape[1] > 90:
                cv.imwrite(TARGET_FILE_PATH + str(counter) + ".jpeg", i)
                counter = counter + 1
